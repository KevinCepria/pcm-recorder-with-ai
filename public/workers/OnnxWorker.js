...
const downSample = (chunk, inputSampleRate = 48000, outputSampleRate = 16000) => {
    ...
    switch (type) {
        case 'init':
            try {
                ...
                stateTensor = new ort.Tensor('float32', new Float32Array(45304), [45304]);
                attenLimDbTensor = new ort.Tensor('float32', new Float32Array([0.0]), [1]);
        ...
        case 'process':
            ...
            try {
                //DNF3 processing
                const inputTensor = new ort.Tensor('float32', data.pcmData, [data.pcmData.length]);
                ...
                self.postMessage({ type: 'dnf3-processed', data: downsampledData.buffer }, [
                ...
                if (sileroSamples.length >= INPUT_SAMPLE) {
                    ...
                    const input = new ort.Tensor('float32', sileroInput, [1, sileroInput.length]);
                    const inputs = {
                        input,
                        h,
                        c,
